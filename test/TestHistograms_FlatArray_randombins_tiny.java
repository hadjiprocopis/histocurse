...
public	class	TestHistograms_FlatArray_randombins_tiny {
	public	static void main(String[] args){
		...
		if( bin_widths.length != num_dims ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : bin_widths must have "+num_dims+" dimensions."); System.exit(1); }
		if( boundaries_from.length != num_dims ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : boundaries_from must have "+num_dims+" dimensions."); System.exit(1); }
		...
		Histogram<FlatArray<Histobin>> hN;
		...
		)) == null ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : call to make_histogram_and_fill_from_NORMAL_distribution() has failed."); System.exit(1); }
		...
		if( doNotSaveDataToFiles == false ){
			try {
				...
				shitprintw.close();
				System.out.println("TestHistograms_FlatArray_randombins_tiny.java : main() : first histogram written to file '"+outfile+"'.");
			} catch(Exception ex){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : exception was caught:\n"+ex); ex.printStackTrace(); System.exit(1); }
		}
		...
		Histogram<FlatArray<Histobin>> regeneratedHist;
		...
		)) == null ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : call to populate_histogram_from_existing_histogram_and_save() has failed."); System.exit(1); }
		...
		try {
			System.out.println("TestHistograms_FlatArray_randombins_tiny.java : similarity between original and created histograms is:"
		...
		Histogram<FlatArray<Histobin>>	regeneratedHist_fix1D = null,
		...
		try {
			...
			if( ! sampler_fix1D.select_bins_using_wildcard_labels(bin_selector_fix1D) ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : call to select_bins_using_wildcard_labels() has failed for selector: "+HistogramSampler.print_bin_selector_spec(bin_selector_fix1D)); System.exit(1); }
			...
			if( ! sampler_fix2D.select_bins_using_wildcard_labels(bin_selector_fix2D) ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : call to select_bins_using_wildcard_labels() has failed for selector: "+HistogramSampler.print_bin_selector_spec(bin_selector_fix2D)); System.exit(1); }
			...
			)) == null ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : call to populate_histogram_from_existing_histogram_and_save() has failed (2)."); System.exit(1); }
		...
		} catch(Exception ex){ System.out.println("TestHistograms_FlatArray_randombins_tiny.java : main() : exception was caught:\n"+ex); ex.printStackTrace(); System.exit(1); }
		System.out.println("TestHistograms_FlatArray_randombins_tiny.java : done, total time taken : "+((System.nanoTime()-nanotime_started)/1000000)+" milli seconds.");
	}

	private	static	Histogram<FlatArray<Histobin>> make_histogram_and_fill_from_NORMAL_distribution(
		...
		long	nanotime_started = System.nanoTime();
		Histogram<FlatArray<Histobin>> ahist = make_histogram(histogram_name, numBins, binWidths, boundariesFrom);
		...
		if( out_basename != null ){
			...
			if( ! save_histogram_for_plotting(ahist, outfile) ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java: make_histogram_and_fill_from_NORMAL_distribution() : call to save_histogram_for_plotting() has failed for output file '"+outfile+"'."); return null; }
		}
	...
	private	static	void	print_distribution_to_file(
		...
		try {
			...
			out.close();
		} catch(Exception ex){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java : print_distribution_to_file() : caught exception : "+ex); ex.printStackTrace(); }
	}

	private	static	void	print_histogram(Histogram<FlatArray<Histobin>> ahist){
		System.out.println(ahist.toString());
	}
	private	static	boolean	save_histogram_for_plotting(
		Histogram<FlatArray<Histobin>> ahist,
		String	outfile
	){
		try {
			ahist.save_to_file_txt(outfile);
		} catch(Exception ex){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java: save_histogram_for_plotting() : call to save_to_file_txt() has failed:\n"+ex); ex.printStackTrace(); return false; }
		return true;
	}
	...
	private	static	Histogram<FlatArray<Histobin>>	make_histogram(
		String histogram_name,
		int[] numBins,
		double[]	binWidths,
		double[]	boundariesFrom
		...
		long	nanotime_started = System.nanoTime();
		Histogram<FlatArray<Histobin>> hist = null;

		try {
			hist = new Histogram<FlatArray<Histobin>>(
				histogram_name,
				// null labels (null means default labels=>coordinates)
				numBins,
				binWidths,
				boundariesFrom,
				(Class<FlatArray<Histobin>> )(Class<?> )FlatArray.class
			);
		} catch(Exception ex){
			System.err.println("TestHistograms_FlatArray_randombins_tiny.java : main() : exception was caught when called new HistogramNDFlatArray() : "+ex);
			ex.printStackTrace();
			return null;
		}
	...
	private static  Histogram<FlatArray<Histobin>>     populate_histogram_from_existing_histogram_and_save(
		String histogram_name,
		Histogram<FlatArray<Histobin>> ahist,
		int     num_samples,
		String	out_basename // leave null if don't want to write out data
		...
		long	nanotime_started = System.nanoTime();
		Histogram<FlatArray<Histobin>> newhist = ahist.clone_only_structure(histogram_name);
...
System.out.println("populate_histogram_from_existing_histogram_and_save() : extracted this value from original histogram and will use it to populate the new hist: "+Arrays.toString(arv));
		...
		if( out_basename != null ){
			String outfile = out_basename + ".txt";
			if( ! save_histogram_for_plotting(newhist, outfile) ){ System.err.println("TestHistograms_FlatArray_randombins_tiny.java: populate_histogram_from_existing_histogram_and_save() : call to save_histogram_for_plotting() has failed for output file '"+outfile+"'."); return null; }
		}
