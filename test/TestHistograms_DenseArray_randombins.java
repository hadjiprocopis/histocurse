...
public	class	TestHistograms_DenseArray_randombins {
	public	static void main(String[] args){
		...
		if( bin_widths.length != num_dims ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : bin_widths must have "+num_dims+" dimensions."); System.exit(1); }
		if( boundaries_from.length != num_dims ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : boundaries_from must have "+num_dims+" dimensions."); System.exit(1); }
		...
		)) == null ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : call to make_histogram_and_fill_from_NORMAL_distribution() has failed."); System.exit(1); }
		...
		if( doNotSaveDataToFiles == false ){
			try {
				...
				shitprintw.close();
				System.out.println("TestHistograms_DenseArray_randombins.java : main() : first histogram written to file '"+outfile+"'.");
			} catch(Exception ex){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : exception was caught:\n"+ex); ex.printStackTrace(); System.exit(1); }
		}
		...
		)) == null ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : call to populate_histogram_from_existing_histogram_and_save() has failed."); System.exit(1); }
		...
		try {
			System.out.println("TestHistograms_DenseArray_randombins.java : similarity between original and created histograms is:"
		...
		try {
			...
			if( ! sampler_fix1D.select_bins_using_wildcard_labels(bin_selector_fix1D) ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : call to select_bins_using_wildcard_labels() has failed for selector: "+HistogramSampler.print_bin_selector_spec(bin_selector_fix1D)); System.exit(1); }
			...
			if( ! sampler_fix2D.select_bins_using_wildcard_labels(bin_selector_fix2D) ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : call to select_bins_using_wildcard_labels() has failed for selector: "+HistogramSampler.print_bin_selector_spec(bin_selector_fix2D)); System.exit(1); }
			...
			)) == null ){ System.err.println("TestHistograms_DenseArray_randombins.java : main() : call to populate_histogram_from_existing_histogram_and_save() has failed (2)."); System.exit(1); }
		...
		} catch(Exception ex){ System.out.println("TestHistograms_DenseArray_randombins.java : main() : exception was caught:\n"+ex); ex.printStackTrace(); System.exit(1); }
		System.out.println("TestHistograms_DenseArray_randombins.java : done, total time taken : "+((System.nanoTime()-nanotime_started)/1000000)+" milli seconds.");
	}
	...
	private	static	Histogram<DenseArray<Histobin>> make_histogram_and_fill_from_NORMAL_distribution(
		...
		if( out_basename != null ){
			...
			if( ! save_histogram_for_plotting(ahist, outfile) ){ System.err.println("TestHistograms_DenseArray_randombins.java: make_histogram_and_fill_from_NORMAL_distribution() : call to save_histogram_for_plotting() has failed for output file '"+outfile+"'."); return null; }
		}
	...
	private	static	void	print_distribution_to_file(
		...
		try {
			...
			out.close();
		} catch(Exception ex){ System.err.println("TestHistograms_DenseArray_randombins.java : print_distribution_to_file() : caught exception : "+ex); ex.printStackTrace(); }
	}
	...
	private	static	boolean	save_histogram_for_plotting(
		...
		} catch(Exception ex){ System.err.println("TestHistograms_DenseArray_randombins.java: save_histogram_for_plotting() : call to save_to_file_txt() has failed:\n"+ex); ex.printStackTrace(); return false; }
		return true;
	}
	...
	private	static	Histogram<DenseArray<Histobin>>	make_histogram(
		...
		try {
			...
			System.err.println("TestHistograms_DenseArray_randombins.java : main() : exception was caught when called new HistogramNDDenseArray() : "+ex);
			ex.printStackTrace();
			return null;
		}
	...
	private static  Histogram<DenseArray<Histobin>>     populate_histogram_from_existing_histogram_and_save(
		...
		if( out_basename != null ){
			String outfile = out_basename + ".txt";
			if( ! save_histogram_for_plotting(newhist, outfile) ){ System.err.println("TestHistograms_DenseArray_randombins.java: populate_histogram_from_existing_histogram_and_save() : call to save_histogram_for_plotting() has failed for output file '"+outfile+"'."); return null; }
		}
